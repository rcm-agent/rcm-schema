-- =====================================================
-- Workflow Table Consolidation Migration (FIXED)
-- Handles missing org_id in workflow_trace
-- Run this with: psql -U postgres -d rcm_db -f consolidate_workflow_tables_fixed.sql
-- =====================================================
...
CREATE TABLE IF NOT EXISTS user_workflow_run (
    ...
    -- LLM tracking (from old workflow_trace - these columns may not exist)
    llm_prompt      TEXT,
    llm_response    TEXT,
    llm_model       VARCHAR(100),
    llm_tokens_used INTEGER,
    
    -- Tier system (may not exist in old tables)
    tier            SMALLINT,
    tier_reason     TEXT,
...
BEGIN
    IF EXISTS (SELECT 1 FROM information_schema.tables 
        ...
        -- Handle missing org_id by getting it from user_workflow
        ...
        SELECT 
            ...
            CASE 
                WHEN wt.status::text = 'pending' THEN 'pending'
                WHEN wt.status::text = 'active' THEN 'running'
                WHEN wt.status::text = 'completed' THEN 'completed'
                WHEN wt.status::text = 'failed' THEN 'failed'
                WHEN wt.status::text = 'cancelled' THEN 'cancelled'
                WHEN wt.status::text = 'timeout' THEN 'timeout'
                ELSE 'pending'
            END,
            wt.channel::text,
            wt.external_id,
            ...
            wt.config_snapshot,
            -- Merge endpoints from workflow_trace_endpoint (if table exists)
            CASE 
                WHEN EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'workflow_trace_endpoint') THEN
                    COALESCE(
        ...
        IF EXISTS (SELECT 1 FROM information_schema.tables 
            ...
            SELECT 
                ...
                ws.step_number,
                CASE 
                    WHEN ws.status::text = 'pending' THEN 'pending'
                    WHEN ws.status::text = 'running' THEN 'running'
                    WHEN ws.status::text = 'completed' THEN 'completed'
                    WHEN ws.status::text = 'failed' THEN 'failed'
                    WHEN ws.status::text = 'skipped' THEN 'skipped'
                    ELSE 'pending'
                END,
                ...
                CASE 
                    WHEN EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'workflow_trace_screenshot') THEN
                        COALESCE(
                            (SELECT jsonb_agg(
                                jsonb_build_object(
                                    'url', screenshot_url,
...
SELECT 
    ...
    external_id,
    CASE 
        WHEN status = 'running' THEN 'active'::trace_status
        ELSE status::trace_status
    END as status,
    config_snapshot,
...
-- ============================================================
-- 6. Enable Row Level Security on new tables (optional)
-- ============================================================

-- Only enable if we have org_id properly set
